...
//  const { importEmployees } = useImportEmployees(); // ✅ Extract correctly
  ...
  const filteredEmployees = clients?.data?.filter((client) => {
    ...
    // Apply search filter
    const matchesSearch = client[filterBy].toString().toLowerCase().includes(searchQuery.toLowerCase());
  ...
  <button
    ...
    className={`px-4 py-2 rounded-md ${selectedClientType === "Hired on Upwork" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}
  >
    Upwork
  </button>
  <button
    onClick={() => setSelectedClientType("Direct")}
    className={`px-4 py-2 rounded-md ${selectedClientType === "Direct" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}
  >
    Direct
  </button>
</div>
  ...
  <thead className="bg-gray-50">
    <tr>
      ...
      {filteredEmployees?.some(client => client.client_type === "Direct") && (
        <>
          <th className="px-6 py-4 font-medium text-gray-600 text-start text-sm">Company Name</th>
          <th className="px-6 py-4 font-medium text-gray-600 text-start text-sm">Address</th>
        </>
      )}
      <th className="px-6 py-4 font-medium text-gray-600 text-start text-sm">Created At</th>
        ...
        <td className="px-6 py-4">
          <div className="flex items-center space-x-2">
            {editingClient === client.id ? (
              <>
                <button
                  ...
                  className="inline-flex items-center justify-center px-3 py-1.5 bg-green-500 hover:bg-green-600 text-white rounded-md transition"
                >
                  Save
                </button>
                <button
                  onClick={() => setEditingClient(null)}
                  className="inline-flex items-center justify-center px-3 py-1.5 border border-gray-500 text-gray-500 hover:bg-gray-50 rounded-md transition"
                >
                  Cancel
                </button>
              </>
            ...
            <button
              ...
              className="inline-flex items-center justify-center px-3 py-1.5 border border-red-500 text-red-500 hover:bg-red-50 rounded-md transition"
            >
